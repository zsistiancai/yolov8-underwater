...
```bash
python train.py --batch 64 --data data.yaml --weights yolov5s.pt --device 0
```
...
```bash
python train.py --batch 64 --data data.yaml --weights yolov5s.pt --device 0,1
```
...
```bash
python -m torch.distributed.run --nproc_per_node 2 train.py --batch 64 --data data.yaml --weights yolov5s.pt --device 0,1
```
...
```bash
python -m torch.distributed.run --nproc_per_node 2 train.py --batch 64 --data data.yaml --cfg yolov5s.yaml --weights '' --device 2,3
```
...
```bash
python -m torch.distributed.run --nproc_per_node 2 train.py --batch 64 --data data.yaml --cfg yolov5s.yaml --weights '' --sync-bn
```
...
```bash
# On master machine 0
python -m torch.distributed.run --nproc_per_node G --nnodes N --node_rank 0 --master_addr "192.168.1.1" --master_port 1234 train.py --batch 64 --data data.yaml --cfg yolov5s.yaml --weights ''
```

```bash
# On machine R
python -m torch.distributed.run --nproc_per_node G --nnodes N --node_rank R --master_addr "192.168.1.1" --master_port 1234 train.py --batch 64 --data data.yaml --cfg yolov5s.yaml --weights ''
```
...
cp data/data.yaml data/coco_profile.yaml
